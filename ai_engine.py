...
def generate_batch_prompt(portfolio_summary, stocks_data):
    ...
    2. **关注股票诊断(核心)**：必须严格遍历上述【已跟踪但未持仓的股票】每一只股票！
       ...
       - 无论数据是否过长，每一只关注股票必须进行分析，然后将结果也输出到stocks_analysis。
    ...
    {{
        "stocks_analysis": [
            {{
                ...
                "price_range": "建议买卖价格区间 (字符串)，如果action为HOLD，则给出预期止盈价格区间（字符串）",
